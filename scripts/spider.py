...
class Spider(object):
	...
	def get_article_link(self, idx):
		link = self.url
		if 'link' in self.src.entries[idx]:
			link = self.src.entries[idx].link
		return link

	def get_article_date(self, idx):
		date_parsed = time.localtime()
		if 'published_parsed' in self.src.entries[idx]:
	...
	def get_article_description(self, idx):
		description = ''
		if 'summary' in self.src.entries[idx]:
			description = self.src.entries[idx].summary
		elif 'content' in self.src.entries[idx]:
			description = self.src.entries[idx].content[0].value
	...
	def get_article_content(self, idx):
		...
		elif 'summary' in self.src.entries[idx]:
